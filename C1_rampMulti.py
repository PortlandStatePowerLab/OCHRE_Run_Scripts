...
filename = '180109_3_15_RC' # date that's thrown away, num of simulation days, data res, ramp or no ramp control
...
if __name__ == "__main__":
    ...
    for home in homes:
        ...
        if M_LU_weighted_pool:
            M_LU_start = M_LU_weighted_pool.pop()
        else:
            M_LU_start = random.choice(M_LU_bins)
        sched['M_LU_time'] = M_LU_start
    
        # -----------------------------
        # M_S_time and M_LU_duration
        # -----------------------------
        # Conditional shift for 05:45 M_LU
        if M_LU_start == '05:45':
            sched['M_S_time'] = '06:15'
        else:
            sched['M_S_time'] = my_schedule['M_S_time']
    
        # Compute M_LU_duration based on actual M_S_time
        ...
        # Random stagger for M_S_duration
        initial_duration = 4
        if MS_weighted_pool:
        ...
        if E_ALU_weighted_pool:
            E_ALU_start = E_ALU_weighted_pool.pop()
        else:
            E_ALU_start = random.choice(E_ALU_bins)
        ...
        sched['E_S_duration'] = 3 + n  # 3h base + random stagger
        sched['E_S_time'] = my_schedule['E_S_time']  # optional explicit assignment
